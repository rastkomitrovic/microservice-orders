...
create table narudzbenica (broj_narudzbenice integer not null auto_increment, datum_azuriranja datetime, datum_kreiranja datetime not null, ukupno double precision not null, primary key (broj_narudzbenice));
...
alter table stavkanarudzbenice add constraint FK_STAVKA_NARUDZBENICE_NARUDZBENICA foreign key (broj_narudzbenice) references narudzbenica (broj_narudzbenice);
alter table stavkanarudzbenice add constraint FK_STAVKA_NARUDZBENICE_PROIZVOD foreign key (proizvod_id) references proizvod (id);
